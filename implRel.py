...
class RelationalQueryProcessor(RelationalProcessor, QueryProcessor):
    ...
    def getJournalArticlesInVolume(self, volume, issn_isbn): 
        with connect(self.getDbPath()) as con: 
            SQL ="SELECT A.id, A.publicationYear, A.title, A.publication_venue, B.issue, B.volume FROM Publications AS A JOIN JournalArticle AS B ON A.id == B.id JOIN Venue AS C ON B.id == C.doi WHERE AND B.volume = '" + str(volume) + "' AND C.issn_isbn = '" + issn_isbn + "'"
        return read_sql(SQL, con)
    ...
    def getProceedingsByEvent(self, name):
        name = name.lower()
        with connect(self.getDbPath()) as con: 
            SQL = "SELECT A.id, A.publication_venue, A.publisher, B.event FROM Publications AS A JOIN Proceeding AS B ON A.id == B.id WHERE B.event LIKE '%" + name.lower() + "%'"
        return read_sql(SQL, con)
